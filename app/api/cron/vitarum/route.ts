// changes euricom (add endpoint to get cvs from vitarum)

import { generateOwnFile } from "@/db/files"
import { getApplicationAccessToken } from "@/lib/server/auth"
import vitarum from "../../../../vitarum.json"
import { VitMatteForImageMatting } from "@xenova/transformers"
...
export async function GET() {
  ...
  const resumes = vitarum

  // const formattedResumes = resumes.map((group: any) => {
  //   return {
  ...
  await generateOwnFile(resumes, "vitarum", "json")
